...
class AnalyticsParser:
    ...
    async def save_and_get_data_to_json(self):
        """
        Собирает и возвращает сводную статистику по активности пользователей и отправленным сообщениям для аналитики.

        Возвращаемый словарь содержит:
            - count_users (int): общее количество пользователей,
            - count_users_today (int): количество новых пользователей, зарегистрированных сегодня,
            - count_messages (int): общее количество отправленных сообщений,
            - count_messages_today (int): количество сообщений, отправленных сегодня,
            - count_messages_per_hour (dict): статистику по количеству сообщений, сгруппированную по часам текущего дня, с ключами:
                * 'hours' — список часов (0–23),
                * 'count_messages' — количество сообщений за каждый час.

        В случае возникновения исключений ошибки логируются через logger, выполнение продолжается.

        Returns:
            dict: собранные данные аналитики.
        Raises:
            Exception: В случае ошибок логирует их через logger и продолжает выполнение.
        """
